...
func TestBareIdentBecomesRef(t *testing.T) {
	...
	pl, err := parsePlan(`on s { dir.owner("/opt", owner) }`, map[string]string{"owner": "x"}, nil, defaultSig, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
	st := pl[0].Steps[0]
...
func TestPlanBindingEnrichesBaseVars(t *testing.T) {
	...
	_, err := parsePlan("owner = \"haribo\"\non s { dir.owner(\"/opt\", owner) }", base, nil, defaultSig, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestInterpolationPrecedence(t *testing.T) {
	...
	pl, err := parsePlan(`on s { dir.owner("/opt", "${owner}") }`,
		map[string]string{"owner": "base"}, map[string]string{"owner": "set"}, defaultSig, nil, nil)
	if err != nil {
		t.Fatal(err)
	}
