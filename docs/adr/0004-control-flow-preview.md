...
- Delivered in increments (epic #45): flow with an instruction condition (PR1), named result capture + `if s1.ok/.changed` (PR2), typed read-only questions (PR3), deprecate plan-level `unless` (PR4).
